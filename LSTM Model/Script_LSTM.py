...
# Creating our data matrix so that first column will include latest values and that is going to be predictor column whereas other columns contain past data that is going to be used for predictions.
...
# Splitting data into training and test portions, then converting into the Tensors
...
# To train data, we use data sets rather than sensors, so this function is to convert tensor into data set
...
# Defining loaders for the data sets
train_loader = DataLoader(train_dataset,batch_size=batch_size,shuffle=True)
test_loader = DataLoader(test_dataset,batch_size=batch_size,shuffle=False)

device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

# This is optional, taking batches from x and y
for _, batch in enumerate(train_loader):
    x_batch, y_batch = batch[0].to(device), batch[1].to(device)
    break

# Implementing LSTM block, input and hidden sized are 1 (prediction column number) and 4 (data used for prediction columns) for this example, respectively
...
# Trains our LSTM with the data once, prints the loss
def train_one_epoch():
    model.train(True)
...
# Validates/tests our LSTM with the data once, prints the loss
...
# PyTorch works with GPU but NumPy does not so we move to CPU before filling predicted array
with torch.no_grad():
    predicted = model(X_train.to(device)).to('cpu').numpy()

train_predictions = predicted.flatten()

# After creation of prediction NumPy array, we prepare it to plot, denormalizing and doing the inverse operations of what we did before to get the plain data
